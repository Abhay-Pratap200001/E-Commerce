          ...
          <nav className="flex flex-wrap items-center gap-4 sm:gap-6">
            ...
            {user && (
              <Link to="/cart"
                ...
                <ShoppingCart className="mr-1 text-gray-300 group-hover:text-emerald-400 transition duration-300" size={20}/>

                {/*  Wrap both spans in a fragment */}
                {cart.length > 0 && (
                  <>
            ...
            {user ? (
              <button
                onClick={logout}
                className="flex items-center text-gray-200 hover:text-emerald-400 border border-emerald-700/30 hover:border-emerald-500/40 py-2 px-4 rounded-md font-medium transition duration-300 ease-in-out shadow-sm">
                <LogOut size={18} />
