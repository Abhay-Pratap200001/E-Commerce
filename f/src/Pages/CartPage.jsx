...
const CartPage = () => {
  const { cart, getCardItems } = useCartStore();
          ...
          <motion.div
            ...
            {cart.length === 0 ? ( <EmptyCartUI />) : (
              <div className="space-y-6">
                ...
                {/*sending all cart which is coming from useCartStore*/}
                {cart.map((item) => (<CartItem key={item._id} item={item}/>))}
              </div>
            )}


            {/* if cart length is greater then 0 show all recommended product */}
            {cart.length > 0 && <PeopleAlsoBought />}
          </motion.div>



          {/* if cart length is greater then 0 */}
          {cart.length > 0 && (
            <motion.div
